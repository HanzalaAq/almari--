  ...
  const { data: listings = [], isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.warn('[HomeScreen] Listing fetch failed, using fallback:', error);
        return selectedCategory === 'All'
          ? fallbackListings
          : fallbackListings.filter((item) => item.category === selectedCategory);
      }
      ...
      <View style={[styles.hero, isCompact && styles.heroCompact]}>
        <View style={styles.heroCopy}>
          ...
          <View style={styles.heroActions}>
            {Platform.OS === 'web' ? (
              <>
                {/* Use native DOM onClick (bypasses ScrollView touch issues) with router.push (client-side nav, no page reload) */}
                <div onClick={() => router.push('/search')} style={{ textDecoration: 'none', display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 9, backgroundColor: '#007782', borderRadius: 7, paddingTop: 13, paddingBottom: 13, paddingLeft: 19, paddingRight: 19, cursor: 'pointer' } as any}>
                  <Text style={styles.primaryButtonText}>Start exploring</Text>
                  <Ionicons name="arrow-forward" size={18} color="#fff" />
                </div>
                <div onClick={() => router.push('/sell')} style={{ textDecoration: 'none', display: 'flex', border: '1px solid #007782', borderRadius: 7, paddingTop: 12, paddingBottom: 12, paddingLeft: 19, paddingRight: 19, cursor: 'pointer' } as any}>
